# ASR Compare — 语音识别引擎对比工具

录音或上传音频，同时发给多个语音识别引擎，并排对比结果。
...
- **并发调用**多个引擎，SSE 流式返回结果（先完成的先显示）
- **CER 计算**：输入参考文本，自动计算各引擎的字错率
- **历史记录**：录音、引擎选择、识别结果持久存储在浏览器 IndexedDB
- **引擎信息**：hover 查看模型 ID、描述、官方文档链接
...
│       ├── ali_dashscope.py  # 阿里云（4 个引擎）
│       ├── baidu.py          # 百度（2 个引擎）
│       ├── xunfei.py         # 讯飞（2 个引擎）
│       ├── tencent.py        # 腾讯云（3 个引擎）
│       └── volcengine_seedasr.py  # 火山引擎（4 个引擎）
├── frontend/
│   ├── index.html
│   ├── app.js               # 引擎定义、识别流程、SSE 读取
